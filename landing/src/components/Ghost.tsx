/**
 * Phantom mascot lifted from mobile/assets/phantom-hero.svg — float, blink
 * and mouth-morph animations preserved.
 */
export default function Ghost({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="80 40 240 360" role="img" aria-label="Phantom mascot" className={className}>
      <defs>
        <style>{`
          @keyframes ghost-float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-15px); } }
          @keyframes ghost-shadow-pulse { 0%, 100% { transform: scale(1); opacity: 0.2; } 50% { transform: scale(0.82); opacity: 0.1; } }
          @keyframes ghost-blink { 0%, 92%, 100% { transform: scaleY(1); } 96% { transform: scaleY(0.1); } }
          @keyframes ghost-mouth-morph {
          ...
          .ghost-body { animation: ghost-float 3.2s ease-in-out infinite; }
          .ghost-ground-shadow { animation: ghost-shadow-pulse 3.2s ease-in-out infinite; transform-origin: 200px 385px; }
          .ghost-eyes { animation: ghost-blink 4.2s infinite; transform-origin: 200px 180px; }
          .ghost-mouth-morph { animation: ghost-mouth-morph 11.6s ease-in-out 2s infinite backwards; }
        `}</style>
      ...
      <ellipse
        ...
        opacity="0.2"
      />
      <g className="ghost-body" filter="url(#ghost-shadow)">
        <path
